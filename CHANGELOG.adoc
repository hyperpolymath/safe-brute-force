...
* *State Management*: gen_statem-based state machine for pause/resume
functionality
* *Pattern Generation*: Multiple strategies (wordlist, charset,
sequential, custom)
* *Execution Engine*: Concurrent worker pool with gen_server
* *Result Aggregation*: Filtered output and statistics tracking
* *Checkpoint System*: Save/restore capability for long-running
operations
* *Safety Mechanisms*: Automatic pause every 25 attempts with user
confirmation
...
*Note*: This project follows ethical security testing principles. All
features are designed for authorized testing only.
